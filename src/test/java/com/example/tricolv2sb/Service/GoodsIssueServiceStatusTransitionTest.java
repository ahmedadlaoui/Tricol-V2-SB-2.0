...
class GoodsIssueServiceStatusTransitionTest {
    ...
    void testValidateGoodsIssue_StatusTransitionFromDraftToValidated() {
        ...
        when(goodsIssueLineRepository.findByGoodsIssueId(1L)).thenReturn(Arrays.asList(issueLine1, issueLine2));
        ...
        when(stockLotRepository.findAvailableLotsByProductIdOrderByEntryDate(1L)).thenReturn(Arrays.asList(stockLot1, stockLot2));
        when(stockLotRepository.findAvailableLotsByProductIdOrderByEntryDate(2L)).thenReturn(Arrays.asList(stockLot3));
